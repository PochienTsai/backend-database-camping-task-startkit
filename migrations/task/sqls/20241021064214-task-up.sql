...
-- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
    ...
    -- 2. `王小明` 購買 `21 堂組合包方案`
    -- 3. `好野人` 購買 `14 堂組合包方案`
insert into "CREDIT_PURCHASE" (user_id, credit_package_id, purchased_credits, price_paid) values((select id from "USER" where email = 'wXlTq@hexschooltest.io'),
...
insert into "CREDIT_PURCHASE" (user_id, credit_package_id, purchased_credits, price_paid) values((select id from "USER" where email = 'richman@hexschooltest.io'),
  (select id from "CREDIT_PACKAGE" where name = '14 堂組合包方案'),
  (select credit_amount from "CREDIT_PACKAGE" where name = '14 堂組合包方案'),
  (select price from "CREDIT_PACKAGE" where name = '14 堂組合包方案'));
...
-- 5-1. 新增：請在 `COURSE_BOOKING` 新增兩筆資料：
    ...
    -- 2. 新增： `好野人` 預約 `李燕容` 的課程
        ...
        -- 2. 預約時間`booking_at` 設為2024-11-24 16:00:00
        -- 3. 狀態`status` 設定為即將授課
insert into "COURSE_BOOKING" (user_id, course_id, booking_at, status) values((select id from "USER" where email = 'wXlTq@hexschooltest.io'),
...
select
  ...
  "SKILL".name as "專長名稱"
from "COACH_LINK_SKILL"inner JOIN "SKILL" on "SKILL".id = "COACH_LINK_SKILL".skill_id
...
select
  ...
  count(*) as "銷售數量"from "CREDIT_PURCHASE"inner join "CREDIT_PACKAGE" on "CREDIT_PACKAGE".id = "CREDIT_PURCHASE".credit_package_id
where "CREDIT_PURCHASE".created_at >= '2024-11-01 00:00:00' and "CREDIT_PURCHASE".created_at <= '2024-11-30 23:59:59'group by "CREDIT_PACKAGE".name;
-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
select sum(price_paid) as "總營收"from "CREDIT_PURCHASE"where "CREDIT_PURCHASE".purchase_at >= '2024-11-01 00:00:00'
and "CREDIT_PURCHASE".purchase_at <= '2024-11-30 23:59:59';
-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數
select count(Distinct("COURSE_BOOKING".user_id)) as "預約會員人數"from "COURSE_BOOKING"where "COURSE_BOOKING".created_at >= '2024-11-01 00:00:00'
